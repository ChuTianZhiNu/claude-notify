...
- **权限审批通知** — Claude Code 需要权限审批时推送飞书通知
- **交互提示通知** — Claude 使用 superpowers 等交互插件需要用户确认时推送通知
- **延迟防抖** — 所有通知延迟 30 秒发送，期间用户有操作则自动取消，避免频繁打扰
...
| `notify_on_permission` | 是否在需要审批/交互时通知 | `true` |
| `max_summary_length` | 任务摘要最大长度 | `200` |
| `min_task_duration` | 任务最短通知阈值（秒），低于此值不发通知 | `60` |
| `debounce_seconds` | 延迟发送等待时间（秒），期间用户操作则取消 | `30` |
| `permission_tools` | 触发通知的权限审批工具白名单 | `["Bash", "Write", "Edit"]` |
| `interactive_tools` | 触发通知的交互类工具白名单 | `["AskUserQuestion"]` |
...
Claude 工作...                              ↓
                                          ↓
任务完成   → Stop Hook → 计算时长
              ├─ 短任务（< min_task_duration）→ 跳过
              └─ 长任务 → 写入 pending → 延迟 debounce_seconds → 无操作 → 飞书通知
                                          ↑
需要审批   → Notification Hook → 写入 pending → 延迟 debounce_seconds → 无操作 → 飞书通知
                                          ↑
用户新操作 → UserPromptSubmit Hook → 清除 pending → 通知取消
```
...
- **Permission approval notification** — Push Feishu notification when Claude Code requests permission
- **Interactive prompt notification** — Push notification when Claude uses superpowers or other interactive plugins waiting for user input
- **Delay debounce** — All notifications are delayed by 30 seconds; cancelled automatically if user takes action during the delay
...
| `notify_on_permission` | Notify on permission/interactive prompt | `true` |
...
| `debounce_seconds` | Delay in seconds before sending; cancelled if user acts | `30` |
| `permission_tools` | Whitelist of permission tools that trigger notifications | `["Bash", "Write", "Edit"]` |
| `interactive_tools` | Whitelist of interactive tools that trigger notifications | `["AskUserQuestion"]` |
...
Claude is working...                                ↓
                                                    ↓
Task done    → Stop Hook → Check duration
                ├─ Short task (< min_task_duration) → Skip
                └─ Long task → Write pending → Delay debounce_seconds → No action → Feishu notification
                                                    ↑
Needs approval → Notification Hook → Write pending → Delay debounce_seconds → No action → Feishu notification
                                                    ↑
User action  → UserPromptSubmit Hook → Clear pending → Notification cancelled
```
